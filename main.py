# -*- coding: utf-8 -*-
import os
import sys  # ✅ ДОБАВЛЕНО: нужен для logging.StreamHandler(sys.stdout)
import asyncio
import logging
import aiohttp
import json
...
# ============================================================
# 🔥 КОНВЕРТАЦИЯ ПОЗИЦИЙ (UTF-16 -> Unicode)
# ============================================================
...
def tg_offset_to_py(text: str, tg_offset: int, context: str = "") -> int:
    ...
    while utf16_pos < tg_offset and py_pos < len(text):
        ...
        if DEBUG_FORMATTING:
            logger.debug(f"   [CHAR] py_pos={py_pos}, char={repr(char)}, U+{code:04X}, UTF-16 width={char_width}")
...
def _show_diff(original: str, result: str, prefix: str = ""):
    ...
    if clean_check == original:
        logger.info(f"{prefix}✅ Тексты совпадают после удаления тегов")
        return True
    
    logger.error(f"{prefix}❌ РАСХОЖДЕНИЕ В ТЕКСТАХ:")
    logger.error(f"{prefix}   Длина оригинала: {len(original)}, результат после очистки: {len(clean_check)}")
    
    for i, (a, b) in enumerate(zip(original, clean_check)):
        if a != b:
        ...
        if len(original) != len(clean_check):
            logger.error(f"{prefix}   Тексты идентичны до позиции {min(len(original), len(clean_check))}, далее разница в длине")
    
    diff = difflib.unified_diff(
        original.splitlines(keepends=True),
        clean_check.splitlines(keepends=True),
        fromfile='original',
    ...
    logger.error(f"{prefix}🔣 Hex результата (первые 100 байт): {clean_check[:100].encode('utf-8').hex()}")
...
def format_text(telegram_text: str, entities: list, message_id: int = None) -> str:
    ...
    for idx, entity in enumerate(entities):
        ...
        if py_start >= py_end:
            logger.warning(f"{msg_prefix} ⚠️ Пропущена сущность {e_type}: некорректный диапазон PY[{py_start}:{py_end}]")
            continue
        ...
        logger.info(f"{msg_prefix}   ✅ {e_type}: PY[{py_start}:{py_end}] фрагмент={repr(fragment[:50])}")
        if url:
    ...
    for fmt in formats:
        ...
        if t == 'text_link' and url:
            ...
            logger.info(f"{msg_prefix} 🏷️  text_link: '{txt[:30]}...'")
            ...
            logger.info(f"{msg_prefix} 🏷️  text_mention: user_id={user.get('id')}")
            
        elif t in html_tags:
            open_tag, close_tag = html_tags[t]
            replacement = f"{open_tag}{txt}{close_tag}"
            logger.info(f"{msg_prefix} 🏷️  {t}: '{txt[:30]}...'")
        else:
    ...
    if DEBUG_FORMATTING:
        logger.debug(f"{msg_prefix} 🔍 ВАЛИДАЦИЯ: оригинал={len(telegram_text)}, после очистки={len(clean_check)}")
    
    if clean_check != telegram_text:
        logger.error(f"{msg_prefix} ❌ ВАЛИДАЦИЯ НЕ ПРОЙДЕНА!")
...
class MediaUploader:
    ...
    async def upload_file_only(self, upload_url: str, file_data: bytes, filename: str) -> bool:
        ...
        logger.debug(f"📤 Загрузка файла: {filename} ({len(file_data)} байт)")
    ...
    async def upload_video(self, file_data: bytes, filename: str) -> Optional[str]:
        try:
            ...
            logger.info(f"🎬 Загрузка видео: {safe_name}")
    ...
    async def upload_document(self, file_data: bytes, filename: str) -> Optional[Tuple[str, str]]:
        try:
            safe_name = safe_filename(filename)
            logger.info(f"📄 Загрузка документа: {safe_name}")
    ...
    async def upload_audio(self, file_data: bytes, filename: str) -> Optional[Tuple[str, str]]:
        try:
            safe_name = safe_filename(filename)
            logger.info(f"🎵 Загрузка аудио: {safe_name}")
            upload_info = await self.create_upload("file")
            upload_url = upload_info.get('url')
    ...
    async def upload_voice(self, file_data: bytes, filename: str) -> Optional[str]:
        try:
            safe_name = safe_filename(filename)
            logger.info(f"🎤 Загрузка голосового: {safe_name}")
...
async def forward(message: types.Message):
    ...
    if message.photo or message.video or message.audio or message.voice or message.document:
        ...
        if not attachments:
            logger.warning("⚠️ Медиа найдено, но не удалось загрузить вложения")
            if not final_text:
                logger.warning("⚠️ Нет текста для отправки, пропускаем")
                return 
