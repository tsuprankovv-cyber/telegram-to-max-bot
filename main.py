...
def process_text(text: str, entities: list) -> str:
    """Обработка текста (только форматирование, без заголовков)"""
    if not text:
        return text
...
async def forward(message: types.Message):
    ...
    if message.photo or message.video or message.audio or message.voice or message.document:
        ...
        # Обрабатываем подпись (только форматирование, без заголовков)
        if message.caption:
            text_entities = message.caption_entities or []
            if text and text_entities:
                text = process_text(text, text_entities)
    ...
    if message.text:
        ...
        processed_text = process_text(text, entities)
...
async def start(message: types.Message):
    await message.answer(
        "✅ **ОБЪЕДИНЁННЫЙ БОТ (БЕЗ ЗАГОЛОВКОВ)**\n\n"
        "📋 **ПОДДЕРЖИВАЕТСЯ:**\n"
...
async def main():
    logger.info("🚀 ЗАПУСК БОТА (БЕЗ ЗАГОЛОВКОВ)")
