...
# 🔹 ИСПРАВЛЕНИЕ: Убираем пробелы в URL
telegram_bot = Bot(token=TELEGRAM_TOKEN, session=aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)))
dp = Dispatcher()
...
class MediaUploader:
    def __init__(self, token: str):
        self.token = token
        # 🔹 ИСПРАВЛЕНИЕ: Убрал пробелы в URL
